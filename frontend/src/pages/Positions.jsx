...
function assetTypeLabel(v) {
  if (v === 'fund') return '基金'
  if (v === 'bond') return '债券'
  if (v === 'futures') return '期货'
  if (v === 'gold') return '贵金属'
  return '股票'
}
    ...
    <div className="space-y-12 text-[#1A1A1A] dark:text-zinc-100">
      ...
      <section className="grid gap-4 md:grid-cols-4">
        <div className="card-hover rounded-2xl bg-white p-5 shadow-sm transition-all duration-200 hover:-translate-y-[2px] hover:shadow-md dark:bg-zinc-900">
          <div className="flex items-center justify-between">
            <p className="text-[12px] font-semibold text-[#999] dark:text-zinc-500">总资产</p>
        ...
        <div className="card-hover rounded-2xl bg-white p-5 shadow-sm transition-all duration-200 hover:-translate-y-[2px] hover:shadow-md dark:bg-zinc-900">
          <div className="flex items-center justify-between">
            <p className="text-[12px] font-semibold text-[#999] dark:text-zinc-500">今日盈亏</p>
          ...
          <div className="mt-1 text-[12px] font-semibold text-[#999] dark:text-zinc-500">较昨日</div>
        </div>

        <div className="card-hover rounded-2xl bg-white p-5 shadow-sm transition-all duration-200 hover:-translate-y-[2px] hover:shadow-md dark:bg-zinc-900">
          <div className="flex items-center justify-between">
            <p className="text-[12px] font-semibold text-[#999] dark:text-zinc-500">持仓标的</p>
            <span className="text-[10px] font-bold text-[#CCC] dark:text-zinc-600">ACTIVE</span>
          </div>
          <div className="mt-2 font-mono text-[28px] font-extrabold tabular-nums text-[#1A1A1A] dark:text-white">
            {Math.round(countUp.holdings)}
          </div>
          <div className="mt-1 text-[12px] font-semibold text-[#999] dark:text-zinc-500">个活跃持仓</div>
        </div>

        <div className="card-hover rounded-2xl bg-white p-5 shadow-sm transition-all duration-200 hover:-translate-y-[2px] hover:shadow-md dark:bg-zinc-900">
          <div className="flex items-center justify-between">
            <p className="text-[12px] font-semibold text-[#999] dark:text-zinc-500">胜率</p>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl bg-white p-5 shadow-sm dark:bg-zinc-900">
          <div className="mb-4 flex items-baseline justify-between gap-3">
            <h2 className="text-[14px] font-extrabold text-[#1A1A1A] dark:text-white">资产分布</h2>
            <span className="text-[12px] font-semibold text-[#999] dark:text-zinc-500">总持仓 ¥ {money(pieData.sum)}</span>
          </div>
          <div className="grid gap-5 sm:grid-cols-[240px_1fr] sm:items-center">
            ...
            <div className="min-w-0">
              ...
              {pieData.rows.length > 6 ? (
                <div className="mt-3 text-[12px] text-[#999] dark:text-zinc-500">还有 {pieData.rows.length - 6} 个标的…</div>
              ) : null}
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl bg-white p-5 shadow-sm dark:bg-zinc-900">
          <div className="mb-4 flex items-baseline justify-between gap-3">
            <h2 className="text-[14px] font-extrabold text-[#1A1A1A] dark:text-white">各标的盈亏分布</h2>
            <span className="text-[12px] font-semibold text-[#999] dark:text-zinc-500">已平仓盈亏汇总（MVP）</span>
          </div>
          <div className="h-[240px] w-full">
            <ResponsiveContainer width="100%" height={250}>
              <BarChart data={barData} margin={{ top: 10, right: 10, bottom: 10, left: 10 }}>
                ...
                />
                <Tooltip
                  formatter={(value) => [`¥ ${money(Number(value))}`, '盈亏']}
                  contentStyle={{
                    borderRadius: 12,
                    border: '1px solid #F0F0F0',
                    fontSize: 12,
      ...
      <section>
        <div className="mb-5 flex items-baseline justify-between gap-4">
          <h2 className="text-lg font-extrabold text-[#1A1A1A] dark:text-white">持仓标的</h2>
          <span className="text-[13px] font-normal text-[#999] dark:text-zinc-500">
            共 {assetCards.length} 个标的
          </span>
        </div>
                    ...
                    >
                      <div className="rounded-xl bg-white/60 p-4 dark:bg-zinc-950/20">
                        <div className="mb-3 flex items-center justify-between">
                          <p className="text-[12px] font-semibold text-[#999] dark:text-zinc-500">交易记录</p>
                          <p className="text-[12px] font-semibold text-[#999] dark:text-zinc-500">共 {list.length} 笔</p>
                        </div>
                                    ...
                                    <div className="ml-4 rounded-2xl bg-white p-4 shadow-sm dark:bg-zinc-900">
                                      <div className="flex flex-wrap items-center justify-between gap-2">
                                        <div className="flex items-center gap-2">
                                          ...
                                          {hasReply ? (
                                            <span className="text-[12px] text-[#999] dark:text-zinc-500" title="有追问">
                                              💬
                                            </span>
                                          ) : null}
                                        </div>
                                        <span className="font-mono text-[12px] font-semibold tabular-nums text-[#999] dark:text-zinc-500">
                                          {emotionEmoji(tr.emotion_score)} {confidenceTag(tr.confidence_score)}
                                        </span>
                                      </div>
      ...
      <section className="pt-4">
        <div className="mb-6 mt-12 flex items-center justify-between gap-4">
          <h2 className="text-lg font-extrabold text-[#1A1A1A] dark:text-white">我的观望仓</h2>
          <Link
            to="/shadow"
            className="text-[12px] font-semibold text-[#666] transition-colors hover:text-black dark:text-zinc-400 dark:hover:text-white"
          >
                ...
                <li key={sp.id} className="stagger-in" style={{ animationDelay: `${i * 60}ms` }}>
                  ...
                  >
                    ...
                    <p className="mt-3 text-[13px] text-[#666] dark:text-zinc-300">
                      假设建仓价 <span className="font-mono font-medium text-[#1A1A1A] dark:text-white">{hypo}</span>
                    </p>

                    {scenario ? (
                      <div className="mt-4 rounded-xl bg-[#FAFAFA] p-4 dark:bg-zinc-950/30">
                        <p className="text-[11px] font-semibold text-[#999] dark:text-zinc-500">情景推演</p>
